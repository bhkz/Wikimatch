...
export default function MatchHero({ match }: { match: MatchContext }) {
  ...
  // Custom states mapping
  let title = "COMMENT CE MATCH\nS'EST ÉCRIT\nSUR WIKIPÉDIA.";
  let subtitle = "Résultat intégré, éditions comparées, incident raconté différemment : voici les histoires fictives publiées autour de cette rencontre.";
  
  if (match.state === "live") {
    title = "COMMENT CE MATCH\nENTRE DANS WIKIPÉDIA.";
    subtitle = "WikiMatch observe les articles suivis. Aucune nouvelle histoire n'est publiée tant qu'un changement substantiel n'est pas vérifié.";
  } else if (match.state === "pre_match") {
    title = "COMMENT CE MATCH\nSERA SUIVI\nSUR WIKIPÉDIA.";
    subtitle = "Le dispositif de surveillance est prêt. Dès le coup d'envoi, WikiMatch documentera les mises à jour et les divergences entre les différentes éditions de l'encyclopédie.";
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6 relative">
          <div className="flex flex-col gap-4 items-start">
            ...
            <div className="font-mono text-[10px] sm:text-xs text-blue-electric uppercase tracking-widest font-bold">
              DOSSIER MATCH · COUPE DU MONDE 2026 {match.state === "live" && "· EN COURS"} {match.state === "pre_match" && "· À VENIR"}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 mt-8">
          ...
          <div className="font-mono text-sm uppercase tracking-widest font-bold mt-4 flex items-center gap-3">
             ...
             <span className={match.state === "live" ? "text-red-signal" : "text-blue-electric"}>
               {match.state === "live" ? `${match.minute}` : match.finalStatus}
             </span>
          </div>
        </div>
            ...
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl uppercase tracking-wide leading-[0.9] text-white">
               {title.split('\n').map((line, i) => (
                  <span key={i} className="block overflow-hidden pb-1">
                    <motion.span initial={{ y: "100%" }} animate={{ y: "0%" }} transition={{ delay: 0.8 + (i * 0.1), duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="block">
                      {line}
                    </motion.span>
                  </span>
               ))}
            ...
            {match.state === "post_match" ? (
               ...
               <div className="bg-navy border border-cream/20 p-4 w-full md:w-64 flex items-center justify-center">
                 <span className="font-mono text-[10px] text-cream/50 uppercase tracking-widest text-center">
                   Écriture en cours...<br/>Flux sous surveillance
                 </span>
               </div>
            ) : null}
            {match.state === "post_match" && (
                <button className="border border-cream/20 text-cream px-8 py-4 font-bold uppercase font-mono tracking-widest text-sm hover:bg-cream/10 transition-colors w-full md:w-auto text-center">
