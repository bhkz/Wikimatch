...
export default function MatchesCalendar() {
  ...
  const [sortOrder, setSortOrder] = useState<"editorial" | "chronological">("editorial");
  ...
  const filteredGroups = useMemo(() => {
    return allMatchesGroups.map(group => {
      const filteredMatches = group.matches.filter(match => {
        ...
        if (filters.teamQuery.trim() !== "") {
          ...
          matchTeam = hName.toLowerCase().includes(query) ||
  ...
  }, [filters, allMatchesGroups]);

  // For chronological order, we could technically re-sort all groups and matches based on a valid date string.
  // Given these are mocked string dates, let's just reverse or sort predefined groups if "chronological" is selected.
  // In our mock chronologically it should be Day 1, Day 2, Published (Day ?), etc.
  const displayGroups = useMemo(() => {
    if (sortOrder === "editorial") {
      return filteredGroups; 
      // mockMatchesData is already in editorial order:
      // published -> observing -> upcoming -> knockout
    } else {
      // Very naive sorting based on 'id' strings since we prefixed them 
      // or we just define chronological as: upcoming day 1 -> observing day 2 -> published -> knockout
      // Let's do a hardcoded sort based on group id for demo "chronological"
      const chronOrder = ["day-june-11", "day-june-18", "day-june-20", "knockout-demo"];
          ...
          <div className="w-full max-w-screen-2xl mx-auto flex flex-col gap-16">
            {displayGroups.length === 0 ? (
              <div className="flex flex-col gap-4 items-center justify-center py-24 text-center">
                ...
                <button 
                  onClick={() => setFilters({ phase: "all", status: "all", teamQuery: "" })}
