/**
 * OG/SEO dynamiques (spec §14) — servi AUX CRAWLERS uniquement via les
 * rewrites conditionnés au User-Agent dans vercel.json. Les humains reçoivent
 * la SPA normale. Retourne un HTML minimal : balises OG + redirection douce.
 */
...
function page(title: string, description: string, path: string): string {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (matchRoute) {
      ...
      if (m) {
        const score =
          m.score_home !== null ? `${m.score_home}–${m.score_away}` : "à venir";
      ...
      title = `La nuit du ${path.slice(6)}`;
      description = "Ce que les matchs de la nuit ont changé sur la carte du monde.";
    }
  } catch {
    // OG par défaut si la DB est indisponible — jamais de 500 pour un crawler.
  }

  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.setHeader("Cache-Control", "public, s-maxage=300, stale-while-revalidate=600");
  res.status(200).send(page(title, description, path));
}
